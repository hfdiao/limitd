...
const StatusResponse = require('../../messages/protocol_buffers').StatusResponse;
const agent = require('auth0-instrumentation');

function build_status_response (message, items) {
  const statusResponse = StatusResponse.create({
  ...
  const response = Response.create({
    ...
    '.limitd.StatusResponse.response': statusResponse
  });
